...
class UpcardController extends CommonController {
    ...
    Public function signUp(){
        if(IS_POST){
            $params = array(
                'uid' => (int)session('uid'),
                'signTime' => strtotime(preg_replace('/\//is','-',I('signDate',0))),
                'upcardTime' => I('cardtime'),             
                'managersignal' => I('manager'),
                'directorsignal' => I('director'),
                'colleagueText' => I('selectstaff','','strip_tags'),
                'reason' => I('nocard'),
            );
            //p($params);die;
            if(is_array($params['upcardTime'])){
                $params['upcardTime'] = implode(';', $params['upcardTime']);
            }
            $upcard = M('upcard');
            if($upcard->add($params)){
                // p($upcard->getLastSql());die;
                $data = array('status'=>1,'info'=>'提交成功');
            }else{

                $data = array('status'=>0,'info'=>$upcard->getError());
            }
    ...
    Public function managersign(){
        ...
        $id=I('id',0,'intval');
        $result=M('upcard')->where(array('id'=>$id))->setField(array('mstatus' => $mstatus));
        if($result){
            $data=array('status'=>1,'info'=>'提交成功');
        }else{
    ...
    Public function directorsign(){
        ...
        $id=I('id',0,'intval');
        $result=M('upcard')->where(array('id'=>$id))->setField(array('dstatus' => $dstatus));
        if($result){
            $data=array('status'=>1,'info'=>'提交成功');
        }else{
            $data=array('status'=>0,'info'=>'提交失败');
    ...
    Public function alists(){
        $uid=session('uid');
        $where=null;
        $getm=M('upcard')->getField('managersignal',true);
    ...
    Public function blists(){
        $uid=session('uid');
        $where=null;
        $getd=M('upcard')->getField('directorsignal',true);
    ...
    Private function _plist($where,$uid){
        $field=Array('id','m_user.uid','signtime','upcardtime','managersignal','directorsignal','reason',
            'mstatus','dstatus','username');
        $upcard = M('upcard'); 
        
        $count      = $upcard->where($where)->count(); 
        $Page       = new \Think\Page($count,8);
        $show       = $Page->show();
        $list = $upcard->where($where)->join('m_user ON m_upcard.uid=m_user.uid')->field($field)
