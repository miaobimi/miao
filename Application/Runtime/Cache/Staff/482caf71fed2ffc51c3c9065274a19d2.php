...
<style>
                                    ...
                                    <?php if(empty($list)): ?><tr ><div class="alert alert-warning" role="alert">没有记录</div></tr>   
                                        ...
                                        <tbody>
                                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                                    ...
                                                    <td><?php if($vo['enddate'] == 0): ?>待确认
                                                    <?php else: echo (date("Y-m-d H:i:s",$vo["enddate"])); endif; ?>
                                                    </td>
                                                    <td><?php echo (msubstr($vo["outreason"],0,15)); ?><button reason="<?php echo ($vo["outreason"]); ?>" class="btn btn-info radius btn-xs reasontip">查看</button></td>
                                                    <td>
                                                        <?php if($vo['expectedreason'] == null): ?>待填
                                                        <?php else: echo (msubstr($vo["expectedreason"],0,15)); ?><button reason="<?php echo ($vo["expectedreason"]); ?>" class="btn btn-info radius btn-xs exreasontip">查看</button><?php endif; ?>
                                                    </td>
                                                    <td><?php echo (date("Y-m-d H:i:s",$vo["signdate"])); ?></td>
                                                    <td>
                                                        <?php if($vo['mstatus'] == 1): ?><span class="btn btn-info btn-circle" ><i class="icon-ok"></i></span>
                                                    ...
                                                    <td>
                                                        <?php if($vo['dstatus'] == 1): ?><span class="btn btn-info btn-circle" ><i class="icon-ok"></i></span>
                                                        <?php else: if($userid == $vo['directorsignal']): ?><button value="<?php echo ($vo["id"]); ?>" id="dbtn" class="btn btn-primary">审核</button>
                                                        <?php else: ?>
                                                        <span class="btn btn-danger btn-circle"><i class="icon-remove"></i></span><?php endif; endif; ?>
                                                    </td>
                                                    <td>
                                                        <?php if($vo['status'] == 1): if(($vo['mstatus'] == 1) AND ($vo['dstatus'] == 1)): ?><button class="btn btn-primary">已审核</button>
                                                            <?php else: ?>
                                                            <button class="btn btn-warning">待审核</button><?php endif; ?> 
                                                        <?php else: ?>
                                                            <a href="<?php echo U('Out/index',array('id'=>$vo['id']));?>" class="btn btn-danger">回岗</a><?php endif; ?>
                                                    </td>
    ...
    <script>
        //导航高亮
        highlight_subnav('<?php echo U("Out/lists");?>');
        ...
        $(document).ready(function () {
            ...
            $('.exreasontip').bind('click',function(){
                var reason = $.trim($(this).attr('reason'));
                layer.alert(reason)
            });
        });
