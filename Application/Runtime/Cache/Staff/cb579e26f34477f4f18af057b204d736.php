                                    ...
                                    <div class="col-sm-12">
                                        <div class="ibox float-e-margins">
                                            ...
                                            <form method="POST" action="<?php echo U('Auth/toGroup',array('gid'=>I('gid')));?>" class="form-horizontal" role="form">
                                                <?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="alert alert-success">
                                                    <h4>
                                                        <label>
                                                         <input class="i-checks" <?php if(in_array($vo['uid'], $authUser)){echo "checked='true'";} ?> type="checkbox" name="userid[]" value="<?php echo ($vo["uid"]); ?>"> <?php echo ($vo["username"]); ?>
                                                         <small class="">最后登录时间：<?php echo (date('Y-m-d H:i:s',$vo["last_login_time"])); ?></small>
                                                        </label>
                                                    </h4>
