...
<style>
                                ...
                                <div class="row">
                                   <div class="col-sm-5 m-b-xs">
                                        <form class="navbar-form navbar-left" role="search">
                                            <select name="groups" class="input-sm form-control input-s-sm inline">
                                                 <option value="<?php echo U('lists',array('gid'=>0));?>">--管理员分组--</option>
                                                  <?php if(is_array($group)): $i = 0; $__LIST__ = $group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option <?php if($vo['id'] == I('gid')): ?>selected="selected"<?php else: ?>value="<?php echo U('lists',array('gid'=>$vo['id']));?>"<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                            </select>
                                        </form>
                                    </div>
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
                                                  <td userid="<?php echo ($vo["uid"]); ?>"><input class="i-checks" type="checkbox" name='pick' /></td>
                                                  ...
                                                  <td align="center" userid="<?php echo ($vo["uid"]); ?>">
                                                    <a href="<?php echo U('User/index',array('uid' => $vo['uid']));?>" class="btn btn-info btn-sm">修改</a>
                                                    <a onclick="del(<?php echo ($vo['uid']); ?>,this)" class="btn btn-danger btn-sm">删除</a>
                                                    <?php if($vo["status"] == 0): ?><a class="btn btn-info btn-sm deny">启用</a><?php else: ?><a class="btn btn-danger btn-sm deny"><if condition="$vo.status eq 0">禁用</a><?php endif; ?>
                                ...
                                <div class="ibox-content">
                                    <div role="form" class="form-inline">
                                        ...
                                        <label class="i-checks">
                                            <input type="checkbox" id="pick" ><i></i> 全选/反选</label>
                                        </div>
                                            <a class="btn btn-info btn-sm">修改</a>
                                            <a class="btn btn-danger btn-sm">删除</a>
                                            <a class="btn btn-info btn-sm deny">启用</a>
                                            <a class="btn btn-info btn-sm">授权</a>
                                    </div>
                                </div>
                                <nav>
                                    <ul class="pagination">
    ...
    <script>
        //导航高亮
        highlight_subnav('<?php echo U("User/lists");?>');
        ...
        $(document).ready(function () {
            ...
            $('#pick').on('ifChecked', function(event){
               $("input[name='pick']").iCheck('check');
            });
            $('#pick').on('ifUnchecked', function(event){
               $("input[name='pick']").iCheck('uncheck'); 
            });
            
        });
            
        function del(val,obj){
