...
<style>
                            ...
                            <div class="ibox-content">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        <?php if(empty($list)): ?><tr ><div class="alert alert-warning" role="alert">没有记录</div></tr>   
                                        <?php else: ?>
                                        <thead>
                                            <tr>
                                                <th>签单日期</th>
                                        ...
                                        <tbody>
                                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                                    <td><?php echo (date("Y-m-d H:i:s",$vo["signtime"])); ?></td>
                                                    <td><?php switch($vo["type"]): case "1": ?>事假<?php break;?>
                                                            <?php case "2": ?>公假<?php break;?>
                                                            <?php case "3": ?>病假<?php break; endswitch;?>       
                                                    </td>
                                                    <td><?php echo (date("Y-m-d H",$vo["starttime"])); ?>至<?php echo (date("Y-m-d H",$vo["endtime"])); ?>共<?php echo ($vo["days"]); ?></td>
                                                    <!-- <td><?php if($vo['mstatus'] == 1): ?><span class="btn btn-info btn-circle"><i class="icon-ok"></i></span>
                                                        <?php else: if($userid == $vo['managersignal']): ?><button value="<?php echo ($vo["id"]); ?>" id="mbtn" class="btn btn-primary">审核</button>
                                                        <?php else: ?>
                                                        <span class="btn btn-danger btn-circle"><i class="icon-remove"></i></span><?php endif; endif; ?>
                                                    </td> -->
                                                    <td><?php if($vo['dstatus'] == 1): ?><span class="btn btn-info btn-circle"><i class="icon-ok"></i></span>
                                                        <?php else: if($userid == $vo['directorsignal']): ?><button value="<?php echo ($vo["id"]); ?>" id="dbtn" class="btn btn-primary">审核</button>
                                                        <?php else: ?><span class="btn btn-danger btn-circle"><i class="icon-remove"></i></span><?php endif; endif; ?>
                                                    </td>
                                                    <td><?php echo ($vo["username"]); ?></td>
                                                    <td><?php echo (msubstr($vo["reason"],0,50)); ?><button reason="<?php echo ($vo["reason"]); ?>" class="btn btn-info radius btn-xs reasontip">查看</button></td>
                                                    <td>
                                                    <!-- <td>
                                                        <?php if(($vo['mstatus'] == 1) AND ($vo['dstatus'] == 1)): ?><span class="btn btn-info btn-circle" ><i class="icon-ok"></i></span>
                                                        <?php else: ?>
                                                            <span class="btn btn-danger btn-circle"><i class="icon-remove"></i></span><?php endif; ?>
                                                    </td> -->
    ...
    <script>
        //导航高亮
        highlight_subnav('<?php echo U("Off/blists");?>');
        ...
        $(document).ready(function () {
            ...
            /*$('#mbtn').click(function(){
                layer.confirm('确定审核？',function(){
                    var val=$('#mbtn').val();
                    $.ajax({
                        url : "<?php echo U('Upcard/managersign');?>", 
                            type : "POST", 
                            dataType : "json", 
                            data: {mstatus:1,id:val}, 
                            success : function(result) { 
                                if(result.status==1){
                                    layer.alert(result.info,{con:6});
                                    $('#mbtn').removeClass('btn-danger');
                                    $('#mbtn').addClass('btn-primary');
                                    $('#mbtn').html('已审核');
                                }else{
                                    layer.alert(result.info,{con:5});
                                }
                            } 
                    });
                });
            ...
            $('#dbtn').click(function(){
                layer.confirm('确定审核？',function(){
                    var val=$('#dbtn').val();
                    $.ajax({
                        url : "<?php echo U('Off/directorsign');?>", 
                            type : "POST", 
                            dataType : "json", 
                            data: {dstatus:1,id:val}, 
                            success : function(result) { 
                                if(result.status==1){
                                    layer.alert(result.info,{con:6});
